...
import '../instructor/MarksDashboard.css';

// â”€â”€â”€ DB column mapping â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
const DB_COLUMN = {
  tt1: 'tt1_marks',
  tt2: 'tt2_marks',
  endSem: 'ese_marks',
...
// Draft key for localStorage â€” persists unsaved marks across tab switches
const draftKey = (courseId, examId) => `statcap_marks_draft_${courseId}_${examId}`;

// â”€â”€â”€ Best-of-N logic â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
const computeEffectiveTotal = (pattern, studentMarks) => {
  ...
  for (const q of pattern) {
    ...
    // Not enough answers filled yet â€” effective is incomplete
...
const EditMarks = () => {
  ...
  // â”€â”€â”€ Fetch students â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  useEffect(() => {
    const fetchStudents = async () => {
      try {
        let q = supabase.from('users').select('*').in('user_type', ['trainee', 'student']);
  ...
  // â”€â”€â”€ Build questions from pattern â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  useEffect(() => {
    if (!loading && !course) { navigate('/instructor/create-course'); return; }
  ...
  // â”€â”€â”€ Always fetch fresh marks from Supabase, then layer localStorage draft â”€
  useEffect(() => {
  ...
  // â”€â”€â”€ Mark change handler â€” stores raw value (no clamping), writes draft to localStorage â”€â”€â”€
  const handleMarkChange = (uid, qId, value, maxAllowed) => {
    ...
    // Store raw value â€” over-max entries are shown with a red pill, not auto-clamped
    setMarksData(prev => {
  ...
  const saveMarks = async () => {
    ...
    students.forEach(student => {
      questions.forEach(q => {
        ...
        if (!isNaN(num) && num > q.max) {
          overMaxErrors.push(`${student.name} â€” ${q.label} (entered ${num}, max ${q.max})`);
        }
      });
    });
    ...
    try {
      ...
      // Clear draft â€” marks are now persisted in DB
  ...
  // â”€â”€â”€ CO change handler â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  // â”€â”€â”€ Derived: best-of annotation per parent Q â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    <div className="edit-marks-container">
        ...
        <div>
          ...
          {maxMarks > 0 && (
            <p style={{ margin: '4px 0 0', fontSize: '0.8rem', color: 'rgba(255,255,255,0.5)' }}>
              Max marks: {maxMarks} Â· Best-of logic applied per question group
            </p>
          )}
        </div>
        <button className="save-btn" onClick={saveMarks} disabled={isSaving || questions.length === 0}>
          {isSaving ? 'Savingâ€¦' : 'Save Marks'}
        </button>
      </div>
      ...
      {questions.length === 0 ? (
        <div className="glass-card" style={{ padding: 40, textAlign: 'center', borderRadius: 16, color: '#fff' }}>
          ...
          <button className="statcap-btn" style={{ marginTop: 20 }} onClick={() => navigate(`/instructor/examination/${examId}`)}>Configure Pattern</button>
        </div>
      ) : loadingMarks ? (
        <div className="glass-card" style={{ padding: 40, textAlign: 'center', borderRadius: 16, color: '#fff' }}>
          Loading marks from databaseâ€¦
        </div>
      ) : (
