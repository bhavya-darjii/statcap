...
const StudentLayout = () => {
  ...
  const [studentName, setStudentName] = useState("Officer");
      ...
      try {
        ...
        if (mounted) {
          if (userData && userData.full_name) {
            ...
            setStudentName((user.user_metadata?.full_name || "Officer").split(' ')[0]);
          }
        }
      } catch (error) {
        if (mounted) setStudentName((user.user_metadata?.full_name || "Officer").split(' ')[0]);
      }
